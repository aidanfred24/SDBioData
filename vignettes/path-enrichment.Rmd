...
In this vignette, we will explore how to perform pathway enrichment analysis using the `SDBioData` package in conjunction with the popular `clusterProfiler` package. We will utilize a sample dataset, `hypox_deseq`, which contains differential expression results comparing treatment under hypoxic conditions.
...
Next, to ensure our gene IDs match the standard format expected by the `SDBioData` database, we can use the `convert_id` function. Here our species of interest is human (Species ID: 96).
...
geneList <- setNames(object = hypox_deseq$log2FoldChange, 
                     nm = rownames(hypox_deseq))
...
By leveraging both ORA and GSEA, we can obtain complementary perspectives on the functional changes induced by our experimental conditions. The `SDBioData` functions help seamlessly bridge basic count/results data with advanced pathway analysis libraries.
